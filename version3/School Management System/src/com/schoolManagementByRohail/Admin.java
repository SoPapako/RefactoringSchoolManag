...
public class Admin extends PersonIdentity {
    ...
    void personalRecordList(String adminFeatures) {
        ArrayList<String> recordsList = readPersonalDetailsFromFile();

        if (adminFeatures.equals("1")) {
            addRecord(recordsList);
        } else if (adminFeatures.equals("2")) {
            deleteRecords(recordsList);
        }
    }

    ArrayList<String> readPersonalDetailsFromFile() {
        ArrayList<String> recordsList = new ArrayList<>();
        try {
            FileReader readPersonalIdentity = new FileReader("PersonalDetails.txt");

            for (int i = 0; i < 6; i++) {
                String specific_Detail = Files.readAllLines(Paths.get("PersonalDetails.txt")).get(i);
